            ...
            : base()
        {
            try
            {
                ...
                if (registerResult != eDeviceRegistrationUnRegistrationResponse.Success)
                    CrestronConsole.PrintLine("EISCDump: Failed to register EISC client: {0}", registerResult.ToString());

                eisc.SigChange += (s, e) =>
                {
                    StringBuilder sb = new StringBuilder("\r\n");
                    sb.AppendFormat("EISCDump: Event=\"{0}\", Name=\"{1}\", Number=\"{2}\"", e.Event, e.Sig.Name, e.Sig.Number);
                    switch (e.Event)
                    {
                    ...
                    {
                        case eSigEvent.BoolChange:
                            eisc.BooleanInput[e.Sig.Number].BoolValue = e.Sig.BoolValue;
                            break;
                        case eSigEvent.UShortChange:
                            eisc.UShortInput[e.Sig.Number].UShortValue = e.Sig.UShortValue;
                            break;
                        case eSigEvent.StringChange:
                            eisc.StringInput[e.Sig.Number].StringValue = e.Sig.StringValue;
                            break;
                        case eSigEvent.UShortInputRamping:
                            break;
                    }
